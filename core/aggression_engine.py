...
class AggressionEngine:
    ...
    def calcular_memoria_agressao(self, saldo_agressor, delta, volume):
        self.memoria_agressao.append({"saldo": saldo_agressor, "delta": delta, "volume": volume})
        if len(self.memoria_agressao) > 20:
            self.memoria_agressao = self.memoria_agressao[-20:]
        compras = sum(1 for item in self.memoria_agressao if item["saldo"] > 0 and item["delta"] > 0)
        vendas = sum(1 for item in self.memoria_agressao if item["saldo"] < 0 and item["delta"] < 0)
        ...
        persistencia_compra = round((compras / len(self.memoria_agressao)) * 100, 2)
        persistencia_venda = round((vendas / len(self.memoria_agressao)) * 100, 2)
        score_agressao = round((saldo_total / 100) + (delta_total / 50) + (volume_total / 1000), 2)
    ...
    def detectar_explosao_fluxo(self, saldo_agressor, delta, volume, score_agressao, intensidade_fluxo):
        if saldo_agressor > 850 and delta > 350 and volume > 1200 and score_agressao > 18 and intensidade_fluxo > 1300:
