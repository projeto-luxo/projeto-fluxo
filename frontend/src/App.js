...
export default function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!carregouHistoricoRef.current) {
        ...
        candleSeriesRef.current.update(historicoComGlow[historicoComGlow.length - 1]);
        ...
        if (data.vwap_superior?.length > 0) {
          vwapSuperiorRef.current.update(data.vwap_superior[data.vwap_superior.length - 1]);
        }

        if (data.vwap_inferior?.length > 0) {
          vwapInferiorRef.current.update(data.vwap_inferior[data.vwap_inferior.length - 1]);
        }
      }
      ...
      if (data.tipo_explosao === "BUY EXPLOSION") {
        fundo = "#021b11";
      } else if (data.tipo_explosao === "SELL EXPLOSION") {
        fundo = "#1b0505";
  ...
  return (
    <div
      style={{
        ...
        height: "100vh",
        padding: 10,
        background: "radial-gradient(circle at top, #111a33 0%, #050915 55%, #02040a 100%)",
  ...
  dataInfo.explosao === "BUY EXPLOSION"
    ...
    : "0 0 18px rgba(0,255,200,0.15)",
      ...
      <div style={{ width: 330, marginLeft: 10 }}>
        <Titulo>TRIN FLOW PRO 5.1</Titulo>

        <Radar
          explosao={dataInfo.explosao}
          scoreAgressao={dataInfo.scoreAgressao}
        />
        ...
        <Box color="#263238">SCORE AGRESSÃO: {dataInfo.scoreAgressao}</Box>
...
function Radar({ explosao, scoreAgressao }) {
  let cor = "#263238";
  ...
  if (explosao === "BUY EXPLOSION") {
    cor = "#00ff99";
    texto = "BUY EXPLOSION";
  } else if (explosao === "SELL EXPLOSION") {
    cor = "#ff3333";
    texto = "SELL EXPLOSION";
  } else if (scoreAgressao > 15) {
    cor = "#00ffc8";
    ...
    >
      <div
        style={{
          ...
          animation: "radarPulse 1.3s infinite",
        }}
      />
